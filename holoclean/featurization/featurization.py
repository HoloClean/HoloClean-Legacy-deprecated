...
class Featurizer:
	...
	"""TODO: create a table with all possible values"""
	...
	"""TODO: create a table the featurization"""
	...
	table_featurizer=self._featurization_for_dc(table_featurizer,possible_table_name,table_name)
	table_featurizer=self._featurization_for_init(table_featurizer,possible_table_name,table_name)
	table_featurizer=self._featurization_for_cooncur(table_featurizer,possible_table_name,table_name)
	...
	for p in range (0, len(dc_sql_parts)):
		...
		new_condition=new_dc[p]
		dc="',"+dc_sql_parts[p]+"'"
	
		#query to create the featurization part for the dc
		if p==0:
    ...
    def _featurization_for_init(self,table_featurizer,possible_table_name,table_name):

	"""TODO: create the query for the featurization with initial values"""

	table_attribute_string=self.dataengine.get_schema('T')
        table_attribute=table_attribute_string.split(',')

	#query to add the initial values to the featurization
    ...
    def _featurization_for_cooncur(self,table_featurizer,possible_table_name,table_name):

	"""TODO: create the query for the featurization with co-occurance values"""

	#query to add the co-occurance values to the featurization
	...
	"""TODO: creates the id of the weights for the featurization table"""
