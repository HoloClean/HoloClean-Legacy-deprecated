    ...
    def get_query(self):
        ...
        for attribute in table_attribute:
           if attribute !="index":
               ...
               query_for_featurization+=""" (SELECT distinct @n := @n + 1 as var_index,   possible_table.tid as rv_index,possible_table.attr_name as rv_attr, possible_table.attr_val as assigned_val, concat ( 'INIT=',"""+ table_attribute + """) as feature,'init' AS TYPE,'      ' as weight_id   from """+ self.table_name+ """ as table1, """+ self.possible_table_name+ """ as possible_table where ("""+condition+"""  ) and (possible_table.attr_val=table1."""+attribute +""") ) UNION"""
...
class Signal_cooccur(Featurizer):
    ...
    def get_query(self):   
        ...
        query_for_featurization=""" (SELECT distinct  @n := @n + 1 as var_index, possible_table.tid as rv_index,possible_table.attr_name as rv_attr, possible_table.attr_val as assigned_val, concat (table1.attr_name,'=',table1.attr_val ) as feature,'cooccur' AS TYPE,'        ' as weight_id  from """+ self.table_name1 + """ as table1, """+ self.possible_table_name+ """ as possible_table  where (table1.attr_name != possible_table.attr_name and table1.tid = possible_table.tid  and table1.observed='1'))"""
        return query_for_featurization
...
class Signal_dc(Featurizer):
    ...
    def get_query(self):      
        ...
        for index_dc in range (0, len(dc_sql_parts)):
            ...
            if index_dc ==0:
                query_for_featurization="""(SELECT distinct  @n := @n + 1 as var_index,  possible_table.tid as rv_index,possible_table.attr_name as rv_attr, possible_table.attr_val as assigned_val, concat ( table2.index,"""+ dc+ """) as feature,'FD' AS TYPE ,'       ' as weight_id  from """+ self.table_name +""" as table2, """+ self.table_name+ """ as table1, """+ self.possible_table_name+ """ as possible_table where (("""+new_condition+""") AND (possible_table.tid != table2.index)  and possible_table.tid=table1.index ) )"""
            else:
                #if you have more than one dc
                query_for_featurization+=""" UNION (SELECT distinct  @n := @n + 1 as var_index, possible_table.tid as rv_index,possible_table.attr_name as rv_attr, possible_table.attr_val as assigned_val, concat ( table2.index,"""+ dc+ """) as feature,'FD' AS TYPE ,'       ' as weight_id  from """+ self.table_name +""" as table2, """+ self.table_name+ """ as table1, """+ self.possible_table_name+ """ as possible_table where (("""+new_condition+""") AND (possible_table.tid != table2.index) and possible_table.tid=table1.index ) )"""
