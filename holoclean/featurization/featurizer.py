...
class Signal_cooccur(Featurizer):
    ...
    def get_query(self):
        ...
        query_for_featurization = """ (SELECT  @n := @n + 1 as var_index, possible_table.tid as rv_index,possible_table.attr_name as rv_attr, possible_table.attr_val as assigned_val, concat (table1.attr_name,'=',table1.attr_val ) as feature,'cooccur' AS TYPE,'        ' as weight_id  from """ + self.table_name1 + """ as table1, """ + self.possible_table_name + """ as possible_table  where (table1.attr_name != possible_table.attr_name and table1.tid = possible_table.tid  and table1.observed='1'))"""
        return query_for_featurization
