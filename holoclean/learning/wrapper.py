...
class Wrapper:
    ...
    def set_weight(self):
        ...
        mysql_query = "Create TABLE " + self.dataset.table_specific_name(
            'Weights') + " as (select weight_id,'1' as Is_fixed,'1' as init_val from " + self.dataset.table_specific_name(
            'Feature') + " where TYPE='init' group by weight_id  ) union (select weight_id,'0' as Is_fixed,'0' as init_val from " + self.dataset.table_specific_name(
            'Feature') + " where TYPE!='init' group by weight_id  ); alter table " + self.dataset.table_specific_name(
            'Weights') + " modify weight_id,Is_fixed,init_val INTEGER ;alter table " + self.dataset.table_specific_name(
            'Weights') + " order by weight_id;"
    ...
    def set_variable(self):
        ...
        mysql_query = mysql_query + ";ALTER TABLE " + self.dataset.table_specific_name(
    ...
    def set_Factor_to_Var(self):
        ...
        mysql_query = mysql_query + "(select NULL as factor_to_var_index, variable_index as vid,attr_val,table1.attr_name from  " + self.dataset.table_specific_name(
            'Possible_values') + " as table1, " + self.dataset.table_specific_name(
            'Variable') + " as table2 where  table1.tid=rv_ind and   table1.attr_name=table2.rv_attr)order by vid;ALTER TABLE " + self.dataset.table_specific_name(
            'Factor_to_var') + " MODIFY factor_to_var_index INT AUTO_INCREMENT PRIMARY KEY;"
        self.dataengine.query(mysql_query)
    ...
    def set_factor(self):
        ...
        mysql_query = mysql_query + "(select distinct NULL as factor_index, '4' as FactorFunction, table1.weight_id as weightID, '1' as Feature_Value, '1' as arity, table3.factor_to_var_index as ftv_offest from " + self.dataset.table_specific_name(
            'Feature') + " as table1," + self.dataset.table_specific_name(
            'Variable') + " as table2," + self.dataset.table_specific_name(
            'Factor_to_var') + " as table3 where  table1.rv_index=table2.rv_ind and table1.rv_attr= table2.rv_attr and table3.vid=table2.variable_index and table3.attr_val=table1.assigned_val );ALTER TABLE " + self.dataset.table_specific_name(
            'Factor') + " MODIFY factor_index INT AUTO_INCREMENT PRIMARY KEY;"
        self.dataengine.query(mysql_query)
    ...
    def spark_list_Factor(self):
        ...
        for row in temp:
            tempdictionary = row.asDict()
            Factor_list.append([np.int16(tempdictionary["FactorFunction"]), np.int64(tempdictionary["weightID"]),
